...
import {
  ...
  detailsExplore,
  updateExplore,
} from "../../../services/admin/apiExplore";
import ViewExploreModal from "./components/ViewExploreModal";
...
function Explore() {
  ...
  const fetchExplore = async () => {
    ...
    try {
      ...
      if (!status || !Array.isArray(data))
        message.warning("No explore items found.");
    } catch (err) {
  ...
  const onView = async (record) => {
    if (record) {
      setLoading(true);
      try {
        const { data } = await detailsExplore(record._id);
        setSelectedExplore(data);
        setViewMode(true);
      } catch {
        message.error("Failed to load explore details.");
